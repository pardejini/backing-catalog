...
func TestGetDetailsForCatalogItemReturnsProperData(t *testing.T) {
	...
	// details is an instance of catalogItem that is going to be changed
	// by a pointer when unmarshaling the response payload
	...
	// payload is the fake response from our fake server
	payload, err := ioutil.ReadAll(recorder.Body)
	if err != nil {
		t.Errorf("Error parsing response body: %v", err)
	}
	// Filtering the response and plucking out only the values that match with
	// catalogItem struct
	...
	// From here on we make basic assertions about the response values
...
// Associates getFulfillmentStatus function to the fakeWebClient struct
// to simulate fulfilment service request call, and returns back information
// about a certain sku
